...
root = './vis_center/02691156_500/'
name = 'CoRe' 
obj_file_name = root + name + '.pth.tar'
map_location=torch.device('cpu') 
a = torch.load(obj_file_name, map_location=map_location)


input_point = torch.tensor(a['input_point'])
input_normal =  torch.tensor(a['input_normal'])
full_input =  torch.tensor(a['full_input'])
full_center = torch.tensor(a['full_center'])


point_dis = torch.zeros(64)
normal_angle = torch.zeros(64)
...
generate_point_cloud_full(full_center, root, name, point_dis)
generate_surfel_cloud(input_point, input_normal, root, name, normal_angle)
