...
class MaskSurf(nn.Module):
    def __init__(self, config):
        super().__init__()
        print_log(f'[MaskSurfel] ', logger ='MaskSurf')
        self.config = config
        self.trans_dim = config.transformer_config.trans_dim
    ...
    def forward(self, pts, vis = False, **kwargs):
        ...
        # neighborhood_normal = neighborhood_withnormal[:, :, :, 3:].contiguous()
        ...
        B, M, C = x_rec.shape
        rebuild_points = self.increase_dim(x_rec.transpose(1, 2)).transpose(1, 2).reshape(B * M, -1, 3)  # BM Gs 6
