...
def test(base_model, test_dataloader, args, config, logger=None):
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            # input_vis_point, output_surfels = base_model(points, queue, use_the_queue, vis=True, vis_surfel=True)  
            # input_point, input_normal, full_input = base_model(points, queue, use_the_queue, vis=True, vis_input=True)  
            full_position, vis_position, full_center = base_model(points, queue, use_the_queue, vis=True, vis_surfel=False)

            # output = {
            #     'input_vis_point': input_vis_point.cpu(),  # [26 * 32, 3]
            #     'output_surfels': output_surfels.cpu()  # [26 * 32, 8]
            # }


            # output = {
            #     'input_point': data[:, :, :3].squeeze(),  # [1024, 3]
            #     'input_normal': data[:, :, 3:6].squeeze(),  # [1024, 3]
            #     'full_input': data.squeeze()  # [1024, 6]
            # }


            output = {
                'full_position': full_position.squeeze(),  # [2048, 3]
                'vis_position': vis_position.squeeze(),  # [832, 3]
                'full_center': full_center  # [64, 3]
            }


            # dense_points, vis_points = base_model(points, queue, use_the_queue, vis=True)
            # dense_points, vis_points, centers = base_model(points, queue, use_the_queue, vis=True)
            final_image = []
            # data_path = f'./vis_surfel/{taxonomy_ids[0]}_{idx}'
            data_path = f'./vis_center/{taxonomy_ids[0]}_{idx}'
            if not os.path.exists(data_path):
                os.makedirs(data_path)
            filename = 'CoRe.pth.tar'
